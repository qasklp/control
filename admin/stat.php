                            ...
                            <tbody>
                              <?php
                                for($i=0; $i < count($stat->get_inzone_count($active_event_id)); $i++){
                                  echo "<tr><td style='font-size:22px;' >".$stat->inzone[$i]['zonename']."</td>
                                  <td style='font-size:24px;' class='center'><p><span class='font-weight-bold mr-4 center'>".$stat->get_zone_count($event->get_active_event_id(), $stat->inzone[$i]['zone_id'])."</span></p></td>
                                  <td style='font-size:24px;' class='right'><p><span class='font-weight-bold mr-4'>".$stat->inzone[$i]['inzone']."</span>(".round(($stat->inzone[$i]['inzone']*100)/$stat->get_zone_count($event->get_active_event_id(), $stat->inzone[$i]['zone_id']), 0)." %)</p></td>
                                  </tr>";
                                }
                              ?>
                            </tbody>
...
var chart3 = new CanvasJS.Chart("chartContainer3", {
  ...
  theme: "light2",
  title: {
    text: "Динаміка входу (інтервал 5 хв.)"
  },
  data: [{
    type: "column",
    indexLabel: "{y}",
