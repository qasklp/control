<?php
    ...
    //echo $_GET['event'].' - '.$_GET['hall'].' - '. $_GET['date'].' - '.$_GET['str_event'].' - '.$_GET['import_bars'];
    if($event->add_event($validation->text_validation($_GET['event']), date('Y-m-d H:i:s', strtotime($_GET['date'])), $_GET['hall'], $_GET['str_event'], $_GET['import_bars']) == 1){
        echo 'Захід: "'.$_GET['event'].'" створенно!';
    }else{
        echo 'Помилка при створенні заходу!';
    }
