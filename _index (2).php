...
<html lang="ru" >
    ...
    <div style='width:100%;'>
        <header>
            <h2 style='float:left;'>Подія: <span id='cur_event'><?= $activ_event_name ?></span> (<?=$_SESSION['user_login']?>)</h2>
...
<div class='control_text' style='color:white; font-size: 60px; font-weight: bold; margin: 160px 10px; text-align:center;'><div id="big_logo"><img src="/img/scan_logo.jpg" alt="scan_logo"></div>Система готова до сканування</div>
...
<!--<div class="examples">
	...
	function scan_code(exit){
		if($('#code').val() != ''){
	     	...
	     	$.ajax({
		        ...
		        //alert(data[0].status);
					...
					if(data[0].status == 3){
						...
						if(data[0].fan == 0){
							$text = 'Вихід: '+ data[0].zone +' Ряд: '+ data[0].row +' Місце: ' + data[0].col;
						}

						if(data[0].fan == 1){
							$text = 'Вихід: '+ data[0].zone;
						}
						$('.control_text').html($text);
						$('#code').val(null);
			...
			$('.control_text').html('Штрихкод не введено');
	 	}
	}
	...
	$( document ).ready(function() {
		...
		if(pos2 != -1){
			$('.switch').html('<span style="margin:0 20px 20px 0; font-size:34px; vertical-align: text-bottom;">Вихід</span><input type="checkbox" class="form-check-input" style="width:50px; height:50px; margin-top: 30px; visibility: initial;">');
		}else{
